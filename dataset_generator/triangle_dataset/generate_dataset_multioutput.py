...
def write_to_file(dataset, file):
    with open(file, "w") as f:
        f.write("side_a,side_b,side_c,triangle_type1,triangle_type2,triangle_type3,triangle_type4\n")
        for data in dataset:
    ...
    while equilateral+isosceles+scalane+non_triangle<no_cases:
        ...
        if is_valid_triangle(side_a, side_b, side_c):
            ...
            if type == 1 and equilateral<no_cases/4+spred:
                dataset.append([side_a, side_b, side_c, 1, 0, 0, 0])
                equilateral+=1
            elif type == 2 and isosceles<no_cases/4+spred:
                dataset.append([side_a, side_b, side_c, 0, 1, 0, 0])
                isosceles+=1
            elif type == 3 and scalane<no_cases/4+spred:
                dataset.append([side_a, side_b, side_c, 0, 0, 1, 0])
                scalane+=1
        elif non_triangle<no_cases/4+spred:
            dataset.append([side_a, side_b, side_c, 0, 0, 0, 1])
            non_triangle+=1
...
main(800, 5, "dataset_generator/triangle_dataset/data/dataset_multioutput.csv")